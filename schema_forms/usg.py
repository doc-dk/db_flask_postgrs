...
class AbvsForm(BaseForm):
    ...
    fld_abvs_diagnosis = SelectField(
        "ABVS Diagnosis", choices=MammographyDict.abvs_diagnosis_choice)


class SonoMammographyForm(SectionForm):
    fld_sonomammography_acc = StringField(
        "Accession number of Sono-Mammography", default=tbd)
    fld_sonomammography_date = StringField(
        "Date of examination of Sono-mammography", default=tbd)
    fld_sonomammography_tissue = SelectField(
        "Tissue Composition", choices=MammographyDict.sonomammo_tissue_choice)
    fld_sonomammography_tissue_other = StringField("Other")
    fld_sonomammography_calc = SelectField(
        "Is there any calcification reported?", choices=CommonDict.breast_choice)
    fld_sonomammography_calc_other = StringField("Other")
    fld_sonomammography_calc_type = SelectField(
        "Calcification location", choices=MammographyDict.sonomammo_calc_choice)
    fld_sonomammography_calc_type_other = StringField("Other")
    fld_sonomammography_arch = SelectField(
        "Architectural distortion", choices=CommonDict.breast_choice)
...
fld_sonomammography_skin = SelectField(
    "Type of Skin Changes", choices=MammographyDict.sonomammo_skin_choice)
fld_sonomammography_skin_other = StringField("Other")
fld_sonomammography_edema = SelectField(
    "Edema", choices=CommonDict.breast_choice)
...
fld_sonomammography_lymph_ax = SelectField(
    "Axillary Lymph Nodes", choices=CommonDict.normal_abnormal_choice)
fld_sonomammography_lymph_ax_other = StringField("Other")
fld_sonomammography_lymph_ax_cort = StringField(
    "Cortical thickness of Abnormal Axillary Lymph Nodes", default=tbd)
...
fld_sonomammography_sol_duct_loc = SelectField(
    "Location of Solitary Dilated duct", choices=CommonDict.breast_choice)
...
class SonoMammoMassForm(SectionForm):
    ...
    fld_sono_location_clock = StringField(
        "What is the clock position of mass?", default='enter position only')
    fld_sono_mass_shape = SelectField(
        "Shape of mass", choices=MammographyDict.sono_mass_shape_choice)
...
fld_sono_pect_dist = StringField(
    "Distance from Pectoralis Major (cm)", default=tbd)
