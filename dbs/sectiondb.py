...
class SectionDb(object):
    ...
    def connect(self, url):
        ...
        try:
            ...
            self.log.get_logger().info("Connection to %s.%s for %s opened." %
                                       (self.db_name, self.collection_name, self.form_class))
        except:
            self.log.get_logger().error("Error connecting to database %s: %s" %
                                        (self.form_class, sys.exc_info()))

    def get_folder_items(self, folder_pk):
        db_entries = self.db.find({'$and': [{"folder_pk": folder_pk}, {
                                  'is_delete': False}, {'doc_type': self.doc_type}]})
